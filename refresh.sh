#!/usr/bin/env bash
# Refresh events.json. Kept as a script so systemd never has to quote
# xvfb-run's --server-args.
set -euo pipefail
cd "$(dirname "$0")"
export LIFE_CHROME=${LIFE_CHROME:-/usr/bin/google-chrome}
...
xvfb-run -a --server-args="-screen 0 1400x1000x24" \
    python3 system/automation/scrape.py --all --out "$PWD/data" || rc=$?
